...
public class TestNGCustomReportListener implements IReporter{
        ...
        if (tests.getAllResults().size() > 0) {
            ...
            for (ITestNGMethod method : getMethodSet(tests, suite)) {
                ...
                if (!className.equalsIgnoreCase(lastClassName)) {
                    if (mq > 0) {
                        cq += 1;
                        writer.print("<tr class=" + sq +  style
                                + (cq % 2 == 0 ? "even" : "odd") + sq +">"
                                + "<td");
                        if (mq > 1) {
                            writer.print(" rowspan="+sq + mq + sq);
                        }
                ...
                mq += 1;
                if (mq > 1) {
                    buff.append("<tr class="+sq + style
                            + (cq % 2 == 0 ? "odd" : "even") + sq+">");
                }
                String description = method.getDescription();
                String testInstanceName = resultSet
                        .toArray(new ITestResult[] {})[0].getTestName();
                buff.append("<td><a href="+sq + "#m"
                        + m_methodIndex
                        + sq + ">"
                        + qualifiedName(method)
                        + " "
                        + (description != null && description.length() > 0 ? "(" +sq
                        + description + sq +")"
                        : "")
                        + "</a>"
                        + (null == testInstanceName ? "" : "<br>("
                        + testInstanceName + ")") + "</td>"
                        + "<td class=" +sq+"numi"+sq+" "+  "style="+sq+"text-align:left;padding-right:2em"+sq+">" + firstLine+"<br/></td>"
                        + "<td style="+sq+"text-align:right"+sq+">" + formatter.format(calendar.getTime()) + "</td>" + "<td class="+sq+"numi"+sq+">"
                        + timeConversion(end - start) + "</td>" + "</tr>");

            }
            if (mq > 0) {
                cq += 1;
                writer.print("<tr class="+sq+style+(cq % 2 == 0 ? "even" : "odd")+sq+">" + "<td");
                if (mq > 1) {
                    writer.print(" rowspan="+sq+mq+sq);
                }
                writer.println(">" + lastClassName + "</td>" + buff);
    ...
    private void resultDetail(IResultMap tests) {
        ...
        for (ITestResult result : testResultsList) {
            ...
            writer.println("<h2 id="+sq+"m"+sq + m_methodIndex + sq+">" + cname + ":"
                    + method.getMethodName() + "</h2>");
            Set<ITestResult> resultSet = tests.getResults(method);
            generateResult(result, method, resultSet.size());
            writer.println("<p class="+sq+"totop"+sq+"><a href="+sq+"#summary"+sq+">back to summary</a></p>");
    ...
    private void generateResult(ITestResult ans, ITestNGMethod method,
        ...
        if (hasParameters) {
            ...
            writer.print("<tr class="+sq+"param"+sq+">");
            for (int x = 1; x <= parameters.length; x++) {
                writer.print("<th>Param." + x + "</th>");
            }
            writer.println("</tr>");
            writer.print("<tr class="+sq+"param stripe"+sq+">");
        ...
        if (hasReporterOutput || hasThrowable) {
            if (hasParameters) {
                ...
                if (parameters.length > 1) {
                    writer.print(" colspan="+sq+parameters.length+sq);
                }
                writer.println(">");
            } else {
                writer.println("<div>");
    ...
    protected void generateExceptionReport(Throwable exception, ITestNGMethod method) {
        String sq="\"";
        writer.print("<div class="+sq+"stacktrace"+sq+">");
    ...
    public void generateSuiteSummaryReport(List<ISuite> suites) {
        ...
        if (qty_tests > 1) {
            String sq="\"";
            writer.println("<tr class="+sq+"total"+sq+"><td>Total</td>");
            ...
            writer.println("<td colspan="+sq+"3"+">&nbsp;</td></tr>");
        }
        writer.println("</table>");
    }
    ...
    private void summaryCell(String v, boolean isgood) {
        ...
        writer.print("<td class="+sq+"numi"+sq + (isgood ? "" : "_attn") + sq+">" + v
                + "</td>");
    }
    ...
    private void startSummaryRow(String label) {
        ...
        m_row += 1;
        writer.print("<tr"
                + (m_row % 2 == 0 ? " class="+sq+"stripe"+sq : "")
                + "><td style="+sq+"text-align:left;padding-right:2em"+sq+"><a href="+sq+"#t"+sq
                + m_testIndex + sq+"><b>" + label + "</b></a>" + "</td>");
    ...
    private void tableStart(String cssclass, String id) {
        String sq="\"";
        writer.println("<table cellspacing="+sq+"0"+sq+" cellspacing="+sq+"0"+sq
                + (cssclass != null ? " class=" + cssclass + sq
                : " style="+sq+"padding-bottom:2em"+sq)
                + (id != null ? " id="+sq+id+sq : " "+sq+") "+sq+">"));
        m_row = 0;
    }
    ...
    private void titleRow(String label, int cq, String id) {
        ...
        String sq="\"";
        if (id != null) {
            writer.print(" id="+sq + id + sq);
        }
        writer.println("><th colspan="+sq + cq + sq+">" + label + "</th></tr>");
        m_row = 0;
    }
    ...
    protected void startHtml(PrintWriter out) {
        String sq="\"";
        out.println("<!DOCTYPE html PUBLIC "+sq+"-//W3C//DTD XHTML 1.1//EN"+sq+" "+sq+"http://www.w3.org/TR/xhtml11/DTD/xhtml11.dtd"+sq+">");
        out.println("<html xmlns="+sq +"http://www.w3.org/1999/xhtml"+sq+">");
        out.println("<head>");
        out.println("<title>TestNG Report</title>");
        out.println("<style type="+sq+"text/css"+sq+">");
